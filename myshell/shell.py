...
class MyShell(Cmd):
	...
	def __init__(self):
		...
		self.envVar = {
			"shell": self.currentDir, 
			"PWD": os.getcwd()
		}
		os.environ.clear()				#Creating my own environ
		os.environ.update(self.envVar)	#Clear these to get the long ugly version
	...
	def do_help(self,args):
		if len(args) == 0:
			...
			print("-quit (or 'q') - quits the shell.")
	...
	#shortcut 'q' for quitting the program.
	def do_q(self, quit):
		self.do_quit("quit")
	...
	def default(self,args):
		try:
			...
			print("The argument *" + args + "* is invalid or does not exist") 

if __name__ == '__main__':
	shell = MyShell()
	shell.prompt = shell.currentDir + ' >>> '
	os.environ.update(shell.envVar)
	shell.cmdloop('Welcome. Type help (or ?) to list commands. Enter a command to begin.\n')
